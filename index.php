
<?php 
include './Header.php';
?>


<section class="page-container">

    <div class="intro-box">

        <div class="intro-container">
            <div class="intro-text">
                <h1>Doe mee en word lid van Security chief </h1>
                <p>Wij zijn security chief. Wij proberen u het beste te beschermen tegen cybercrime.
                    Wordt u ook lid van Security Chief?</p>

                <div class="contact_wrap">
                    <p class="intro-contact">Heb je geen account en wil je mee doen dan!! </p>
                    <button typ="button" class="intro_btn"><a href="./contact.php" class="bg-rotate">Contact
                            us</a></button>
                </div>

            </div>
            <div class="introFoto-container">
                <img src="<?php echo $brand_logo_bg;?>" alt="Sorry this image is broken (Please contact the administrator)" class="intro-foto">
            </div>
        </div>
    </div>

    <div class="intro-info-container">
        <div class="info-wraper">
            <div class="info-foto-container">
                <img src="./img/spear-phishing.jpeg" alt="">
            </div>
            <div class="info-text">
                <h1> Phishing </h1>
                <p>Phishing is cybercrime waarbij criminelen proberen inloggegevens, creditcardinformatie, pincodes of
                    andere persoonlijke gegevens van jou te achterhalen. </p>

                <h3>Hoe kan ik het voorkomen?</h3>
                <ul>
                    <li> Reageer niet op het bericht.</li>
                    <li> Meld de phishing bij Fraudehelpdesk </li>
                </ul>


                <button typ="button" class="readBtn-wraper"><a href="./info.php#phishing" class="read-btn">Read more</a></button>
            </div>
        </div>
        <div class="info-wraper">
            <div class="info-foto-container">
                <img src="./img/scam.jpg" alt="">
            </div>
            <div class="info-text">
                <h1> Scam </h1>
                <p>Helaas zullen er altijd mensen zijn die de boel moedwillig oplichten. U betaalt keurig,
                    maar ontvangt vervolgens niets. Of u wordt overgehaald om vast uw product op te sturen,
                    maar ontvangt nooit een betaling. In beide gevallen bent u waarschijnlijk opgelicht.</p>

                <h3>Hoe kan ik het voorkomen?</h3>
                <ul>
                    <li>Doe onderzoek naar de persoon of naar het bedrijf van waar je het koopt.</li>
                    <li>Als je het niet vertrouwd koop het niet. </li>
                </ul>


                <button typ="button" class="readBtn-wraper"><a href="./info.php#scam" class="read-btn">Read more</a></button>
            </div>
        </div>
        <div class="info-wraper">
            <div class="info-foto-container">
                <img src="./img/malware.jpg" alt="">
            </div>
            <div class="info-text">
                <h1> Malware </h1>
                <p>Malware kan computers en apparaten op verschillende manieren infecteren en bestaat in
                    verschillende vormen,
                    zoals virussen, wormen, trojans en spyware.</p>

                <h3>Hoe kan ik het voorkomen?</h3>
                <ul>
                    <li>Wees op je hoeden.</li>
                    <li>Gebruik Beveiligingssoftware. </li>
                </ul>


                <button typ="button" class="readBtn-wraper"><a href="./info.php#malware" class="read-btn">Read more</a></button>
            </div>
        </div>
        <div class="info-wraper">
            <div class="info-foto-container">
                <img src="./img/hacking.jpg" alt="">
            </div>
            <div class="info-text">
                <h1> Hacking </h1>
                <p> Hacking betekent dat iemand toegang probeert te krijgen tot de gegevens van iemand anders,
                    via het gebruik van
                    computers en het netwerk. Het is strafbaar als dit gebeurt zonder toestemming van de
                    eigenaar van die gegevens.</p>

                <h3>Hoe kan ik het voorkomen?</h3>
                <ul>
                    <li>Gebruik sterke wachtwoorden</li>
                    <li>Vermijd dubieuze websites </li>
                </ul>


                <button typ="button" class="readBtn-wraper"><a href="./info.php#hacking" class="read-btn">Read more</a></button>
            </div>
        </div>
        <div class="info-wraper">
            <div class="info-foto-container">
                <img src="./img/ransomware.jpg" alt="">
            </div>
            <div class="info-text">
                <h1> Ransomware </h1>
                <p>Ransomware is malware die de databestanden van gebruikers versleutelt, met als doel om
                    deze later te
                    ontsleutelen in ruil voor losgeld. In extreme gevallen blokkeert de ransomware de
                    toegang tot het IT-systeem
                    door ook systeembestanden te versleutelen die essentieel zijn voor de goede werking van
                    het systeem.</p>

                <h3>Hoe kan ik het voorkomen?</h3>
                <ul>
                    <li>Maak regelmatig back-ups </li>
                    <li>Update je software </li>
                </ul>


                <button typ="button" class="readBtn-wraper"><a href="./info.php#ransomware" class="read-btn">Read
                        more</a></button>
            </div>
        </div>
        <div class="info-wraper">
            <div class="info-foto-container">
                <img src="./img/ddos.jpg" alt="">
            </div>
            <div class="info-text">
                <h1> DDoS aanvallen </h1>
                <p>Bij een DDoS-aanval (Distributed Denial of Service-aanval) krijgt de getroffen server
                    zoveel verzoeken te
                    verwerken dat hij het simpelweg niet meer aan kan. De websites zijn dan niet
                    bereikbaar voor bezoekers.</p>

                <h3>Hoe kan ik het voorkomen?</h3>
                <ul>
                    <li>Wees er vroeg bij om web verkeer te controleren</li>
                    <li>Meer bandbreedte verkrijgen </li>
                </ul>


                <button typ="button" class="readBtn-wraper"><a href="./info.php#DDoS" class="read-btn">Read
                        more</a></button>
            </div>
        </div>

        <div class="info-wraper">
            <div class="info-foto-container">
                <img src="./img/spoofing.jpg" alt="">
            </div>
            <div class="info-text">
                <h1> Spoofing </h1>
                <p>Bij spoofing nemen internetcriminelen een identiteit over. Als zogenaamde bekende of
                    erkende afzender probeert
                    de cybercrimineel achter je persoonsgegevens of je pincode te komen, malware op je
                    computer te
                    plaatsen of je direct geld afhandig te maken.</p>

                <h3>Hoe kan ik het voorkomen?</h3>
                <ul>
                    <li>Stel een google alert in.</li>
                    <li>Voorkom email spoofing </li>
                </ul>


                <button typ="button" class="readBtn-wraper"><a href="./info.php#spoofing" class="read-btn">Read
                        more</a></button>
            </div>
        </div>
        <div class="info-wraper">
            <div class="info-foto-container">
                <img src="./img/id.jpg" alt="">
            </div>

            <div class="info-text">
                <h1> Identiteitsfraude </h1>
                <p>Bij identiteitsfraude maakt iemand misbruik van je persoonlijke gegevens. Onder je
                    naam worden er producten of
                    diensten besteld, uitkeringen of creditcards aangevraagd, betalingen gedaan of
                    bankrekeningen geopend.</p>

                <h3>Hoe kan ik het voorkomen?</h3>
                <ul>
                    <li>Beveilig je computer</li>
                    <li>Gebruik sterke wachtwoorden </li>
                </ul>


                <button typ="button" class="readBtn-wraper"><a href="./info.php#identiteitsfraude" class="read-btn">Read
                        more</a></button>
            </div>
        </div>
        <div class="info-wraper">
            <div class="info-foto-container">
                <img src="./img/crypto.jpg" alt="">
            </div>

            <div class="info-text">
                <h1> Cryptojacking </h1>
                <p> Cryptojacking is een vorm van cybercrime waarbij cybercriminelen geld willen
                    verdienen door cryptogeld in
                    handen te krijgen. Ze besmetten de netwerken en apparaten dan bijvoorbeeld met
                    malware.</p>

                <h3>Hoe kan ik het voorkomen?</h3>
                <ul>
                    <li>Blokkeer javascript in je browser.</li>
                    <li>Gebruik goede antimalware </li>
                </ul>


                <button typ="button" class="readBtn-wraper"><a href="./info.php#cryptojacking" class="read-btn">Read
                        more</a></button>
            </div>

        </div>

    </div>



</section>

</body>


</html>